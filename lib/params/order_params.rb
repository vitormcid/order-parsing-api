      ...
      def call(params)
        params.permit(
          ...
          order_items: [:quantity, :unit_price, :full_unit_price,
                        { item: %i[id title] }], payments: %i[id order_id payer_id
          ...
          shipping: [:id, :shipment_type, :date_created,
                     { receiver_address: [:id, :address_line, :latitude, :longitude,
                                          ...
                                          { city: [:name], state: :name, country: %i[id name],
                                            neighborhood: %i[id name] }] }], buyer: [:id, :nickname,
                                                                                     :email, :first_name, :last_name, { phone: %i[area_code number],
                                                                                                                        billing_info: %i[doc_type
                                                                                                                                         doc_number] }]
        )
      end
    end
